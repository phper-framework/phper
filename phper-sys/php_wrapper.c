...
zend_uchar phper_zval_get_type(const zval *pz) { return zval_get_type(pz); }

void phper_zval_arr(zval *val, zend_array *arr) { ZVAL_ARR(val, arr); }
...
char *phper_z_strval_p(const zval *v) { return Z_STRVAL_P(v); }

zval *phper_get_this(zend_execute_data *execute_data) { return getThis(); }

void phper_zval_zval(zval *val, zval *zv, int copy, int dtor) {
  ZVAL_ZVAL(val, zv, copy, dtor);
}

void phper_zval_copy(zval *val, const zval *zv) { ZVAL_COPY(val, zv); }

void phper_zval_copy_value(zval *val, const zval *zv) {
  ZVAL_COPY_VALUE(val, zv);
}

zend_string *phper_zval_get_string(zval *op) { return zval_get_string(op); }

zend_long phper_zval_get_long(zval *op) { return zval_get_long(op); }
...
void phper_array_init(zval *arg) { array_init(arg); }
...
void phper_zval_obj(zval *z, zend_object *o) { ZVAL_OBJ(z, o); }

void phper_zval_func(zval *z, zend_function *f) { ZVAL_FUNC(z, f); }
...
zend_string *phper_get_function_or_method_name(const zend_function *func) {
  ...
  if (func->common.scope) {
    return phper_zend_string_concat3(
        ...
        "::", sizeof("::") - 1, ZSTR_VAL(func->common.function_name),
...
void phper_zval_ptr_dtor(zval *zv) { ZVAL_PTR_DTOR(zv); }
...
void phper_zval_ptr_dtor_nogc(zval *zval_ptr) { zval_ptr_dtor_nogc(zval_ptr); }

bool phper_z_refcounted_p(zval *zval_ptr) { return Z_REFCOUNTED_P(zval_ptr); }
...
int phper_z_res_handle_p(const zval *val) { return Z_RES_HANDLE_P(val); }

int phper_zstr_len(const zend_string *s) { return ZSTR_LEN(s); }

const char *phper_zstr_val(const zend_string *s) { return ZSTR_VAL(s); }

void *phper_emalloc(size_t size) { return emalloc(size); }

void phper_efree(void *ptr) { return efree(ptr); }

void phper_separate_string(zval *zv) { SEPARATE_STRING(zv); }

void phper_separate_array(zval *zv) { SEPARATE_ARRAY(zv); }

void phper_zval_null(zval *zv) { ZVAL_NULL(zv); }

void phper_zval_true(zval *zv) { ZVAL_TRUE(zv); }

void phper_zval_false(zval *zv) { ZVAL_FALSE(zv); }

void phper_zval_long(zval *zv, zend_long l) { ZVAL_LONG(zv, l); }

void phper_zval_double(zval *zv, double d) { ZVAL_DOUBLE(zv, d); }

uint32_t phper_z_type_info_p(const zval *zv) { return Z_TYPE_INFO_P(zv); }

int phper_z_type_p(zval *zv) { return Z_TYPE_P(zv); }

void phper_zval_str(zval *zv, zend_string *s) { ZVAL_STR(zv, s); }
...
zend_array *phper_z_arr_p(const zval *zv) { return Z_ARR_P(zv); }

zend_long phper_z_lval_p(const zval *zv) { return Z_LVAL_P(zv); }

double phper_z_dval_p(const zval *zv) { return Z_DVAL_P(zv); }

zend_string *phper_z_str_p(const zval *zv) { return Z_STR_P(zv); }

zend_resource *phper_z_res_p(const zval *zv) { return Z_RES_P(zv); }

zend_string *phper_zend_string_copy(zend_string *s) {
  return zend_string_copy(s);
}

const char *phper_get_zend_module_build_id() { return ZEND_MODULE_BUILD_ID; }

void phper_convert_to_long(zval *op) { convert_to_long(op); }

void phper_convert_to_string(zval *op) { convert_to_string(op); }

void phper_zend_object_release(zend_object *obj) { zend_object_release(obj); }

const zend_object_handlers *phper_z_obj_ht_p(const zval *zv) {
  return Z_OBJ_HT_P(zv);
}

zend_object *phper_z_obj_p(const zval *zv) { return Z_OBJ_P(zv); }

uint32_t phper_z_addref_p(zval *zv) { return Z_ADDREF_P(zv); }
...
typedef void (*phper_foreach_func_arg_t)(zend_ulong idx, zend_string *key,
                                         zval *val, void *argument);

void phper_zend_hash_foreach_key_val(zend_array *array,
                                     phper_foreach_func_arg_t f,
                                     void *argument) {
  zend_ulong idx;
  zend_string *key;
  zval *val;
