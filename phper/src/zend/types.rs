use crate::{
    ...
    zend::{api::FunctionEntries, classes::This, compile::Visibility, throws::Throwable},
};
use std::{
    borrow::Cow,
    cell::Cell,
...
impl ExecuteData {
    ...
    pub fn parse_parameters<T: ParseParameter>(&self) -> Option<T> {
        <T>::parse(unsafe { self.num_args() } as usize, ())
    }
    ...
    pub fn parse_parameters_optional<T: ParseParameter, O: OptionalParameter>(
        ...
        <T>::parse(unsafe { self.num_args() } as usize, default)
    }
}
